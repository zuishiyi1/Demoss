...
public class Hero_shangbanshen_Script : MonoBehaviour {
	...
	public GameObject ASRbullet;//子弹
	public Canvas diecanvas;//死亡ui
	public Image diebg;//死亡时出现的背景
	public Image dietext;
	public Button die_tryagain;
	public Button die_exit;
	public GameObject ASRMaincamera;

	private bool flipx;
	private SpriteRenderer sr;
	private AudioSource ASRaudios;
	...
	public GameObject ASRxiabanshen;
	...
	void ShowBullet()
	{
		...
		bullets.transform.parent = ASRbullet.transform.parent;
		...
		if (this.GetComponent<Animator> ().GetFloat ("Vertical") >= 0.5f) {
			...
			ASRbullet.transform.localEulerAngles = rotation;
		}
		//正常站立射击
		else {
	...
	void HideXiabanshen()
	{
		ASRxiabanshen.GetComponent<SpriteRenderer> ().enabled = false;
	}
	...
	void ShowDie()
	{
		...
		if (dietext.GetComponent<CanvasGroup> ().alpha >= 0.9f) {
			die_tryagain.GetComponent<Image> ().enabled = true;
			die_exit.GetComponent<Image> ().enabled = true;
			return;
		}
	...
	{
		if (s == "c") {
			ASRMaincamera.SendMessage ("Reset");
		}
		if (s == "e") {
			Application.Quit ();
		}
	...
	{
		ASRaudios.clip = Resources.Load ("normalgunshoot") as AudioClip;
		ASRaudios.Play ();
	}

	void PlayDieVoice()
	{
		ASRaudios.clip = Resources.Load ("hero_die") as AudioClip;
		ASRaudios.Play ();
	}
